(() => {
  ...
  function updateAimbot(now) {
    ...
    for (let i = 0; i < enemies.length; i++) {
        ...
        if (enemy.kind === "prop" || enemy.kind === "spawnable" || enemy.kind === "object") continue;
        if (enemy.brawlerName === null || enemy.brawlerName === undefined) continue;
        
        
        if (enemy.deadFlag || enemy.isDead) continue;
        if (enemy.health === 0 || enemy.health < 0) continue;
        if (enemy.x === 0 && enemy.y === 0) continue;
        ...
        row.lastSeen = now;
        row.health = enemy.health || row.health || 100;
        row.deadFlag = enemy.deadFlag || false;
        
        _motion(row, enemy.x, enemy.y, now);
    }
 
    for (const [id, row] of _live) {
        if (!seen.has(id)) {
            _live.delete(id);
            continue;
        }
        if (row.deadFlag || row.health <= 0) {
            _live.delete(id);
            continue;
        }
        if (now - (row.lastSeen || 0) > 2000) {
            _live.delete(id);
        }
    }
