...
(() => {
  ...
  function _readOwnerTeam(obj) {
    try {
      return obj.add(offsets.LogicProjectileClient_ownerTeam).readU32() | 0;
    } catch (_) {
      return -1;
    }
  }
  function _isFriendlyShot(ownerIndex, ownerTeam) {
    if (scanData.myPlayerIndex >= 0 && ownerIndex === scanData.myPlayerIndex) return true;
    if (ownerIndex >= 0 && _friendlyIdx.has(ownerIndex)) return true;
    if (ownerTeam >= 0 && ownerTeam === scanData.myTeamId) return true;
    return false;
  }
  function _spawnedOnSelf(x, y) {
    if (!(scanData.myX || scanData.myY)) return false;
    const dx = x - scanData.myX;
    const dy = y - scanData.myY;
    const r = (scanData.myRadius || 60) + 80;
    return dx * dx + dy * dy <= r * r;
  }
  function _dropLiveProjectile(projectile) {
    try {
      _liveProjectiles.delete(String(getFunctions().LogicGameObjectClient_getGlobalID(projectile)));
    } catch (_) {
    }
  }
  ...
  function _captureProjectile(projectile, data, ownerTeamOverride) {
    ...
    try {
      ...
      if (existing) {
        ...
        const ownerIndex2 = _readOwnerIndex(projectile);
        if (ownerIndex2 >= 0) existing.ownerIndex = ownerIndex2;
        if (ownerTeamOverride !== void 0) existing.ownerTeam = ownerTeamOverride | 0;
        else {
          const ownerTeam2 = _readOwnerTeam(projectile);
          if (ownerTeam2 >= 0) existing.ownerTeam = ownerTeam2;
        }
        if (_isFriendlyShot(existing.ownerIndex, existing.ownerTeam)) _liveProjectiles.delete(gid);
        return;
      }
      ...
      const ownerTeam = ownerTeamOverride === void 0 ? _readOwnerTeam(projectile) : ownerTeamOverride | 0;
      const ownerIndex = _readOwnerIndex(projectile);
      if (_isFriendlyShot(ownerIndex, ownerTeam)) return;
      if (ownerIndex < 0 && ownerTeam < 0 && _spawnedOnSelf(x, y)) return;
  ...
  function _removeProjectile(projectile) {
    try {
      ...
      if (record && !_isFriendlyShot(record.ownerIndex, record.ownerTeam) && _destroyed.length < 16) {
        _destroyed.push(
          {
            name: record.name,
            angle: record.angle,
  ...
  function _updateLiveProjectiles(now) {
    ...
    for (const [gid, projectile] of _liveProjectiles) {
      try {
        ...
        const ownerIndex = _readOwnerIndex(projectile.ptr);
        if (ownerIndex >= 0) projectile.ownerIndex = ownerIndex;
        const ownerTeam = _readOwnerTeam(projectile.ptr);
        if (ownerTeam >= 0) projectile.ownerTeam = ownerTeam;
        if (_isFriendlyShot(projectile.ownerIndex, projectile.ownerTeam)) {
          _liveProjectiles.delete(gid);
          continue;
        }
          ...
          for (let i = 0; i < count; i++) {
            try {
              ...
              if (data.readPointer().equals(vtableProj)) {
                ...
                else if (state.autododge) _dropLiveProjectile(obj);
                continue;
              }
  ...
  function _snapshotFps() {
    if (_savedFps) return;
    const fps = _readTarget2();
    if (fps != null && isFinite(fps) && fps > 0 && fps < 9e3) _savedFps = fps;
  }
  function _restoreFps() {
    if (_savedFps) _apply(_savedFps);
    _interval = -1;
  }
  function _sync() {
    ...
    if (swap) {
      Interceptor.attach(
        swap,
        {
          onEnter(args) {
            _swaps++;
            if (!state.fpsunlock) return;
            _snapshotFps();
            _apply(UNLOCKED_FPS);
            if (_interval !== 0) {
              _interval = 0;
              setSwapInterval(args[0], 0);
            }
          }
        }
      );
  ...
  function readIdentity() {
    ...
    for (let i = 0; i < paths.length; i++) {
      const value = String(readText(paths[i]) || "").trim().toLowerCase();
      if (/^[0-9a-f]{64}$/.test(value)) return value;
    }
    return "";
  }
  function apkPathFromMaps() {
    try {
      const text = File.readAllText("/proc/self/maps");
      const lines = text.split("\n");
      let fallback = "";
      for (let i = 0; i < lines.length; i++) {
        const path = lines[i].slice(lines[i].lastIndexOf(" ") + 1).trim();
        if (path.slice(-9) === "/base.apk") return path;
        if (!fallback && path.slice(-4) === ".apk") fallback = path;
      }
      return fallback;
    } catch (_) {
      return "";
  ...
  function oatFiles(apkPath) {
    const slash = apkPath.lastIndexOf("/");
    if (slash < 0) return [];
    const root = apkPath.slice(0, slash);
    const abis = ["arm64", "arm64-v8a", "armeabi-v7a", "x86_64", "x86"];
    const out = [];
    for (let i = 0; i < abis.length; i++) {
      out.push(root + "/oat/" + abis[i] + "/base.vdex");
      out.push(root + "/oat/" + abis[i] + "/base.odex");
    }
    return out;
  }
  function fileContains(path, digest) {
    const needle = asciiBytes(digest);
    try {
      const file = new File(path, "rb");
      let prev = new Uint8Array(0);
      while (true) {
        const chunk = file.read(65536);
        if (!chunk) break;
        if (typeof chunk === "string") {
          if (chunk.indexOf(digest) !== -1) {
            file.close();
            return true;
          }
        ...
        const cur = new Uint8Array(chunk);
        const buf = new Uint8Array(prev.length + cur.length);
        buf.set(prev);
        buf.set(cur, prev.length);
        if (indexOfBytes(buf, needle) !== -1) {
          file.close();
          return true;
        }
        prev = cur.length >= needle.length ? cur.slice(cur.length - needle.length + 1) : cur;
      }
      file.close();
    } catch (_) {
    }
    const text = readText(path);
    return !!(text && text.indexOf(digest) !== -1);
  }
  function mappedOatContains(digest) {
    try {
      const needle = [];
      for (let i = 0; i < digest.length; i++) needle.push(digest.charCodeAt(i).toString(16).padStart(2, "0"));
      ...
      for (let i = 0; i < ranges.length; i++) {
        const path = ranges[i].file && ranges[i].file.path;
        if (!path || path.indexOf("com.supercell.brawlstars") === -1) continue;
        if (path.indexOf(".vdex") === -1 && path.indexOf(".odex") === -1) continue;
        const hits = Memory.scanSync(ranges[i].base, ranges[i].size, pattern);
  ...
  function dexHasIdentity(digest) {
    if (mappedOatContains(digest)) return true;
    const apk = apkPathFromMaps();
    if (!apk) return false;
    const files = oatFiles(apk);
    for (let i = 0; i < files.length; i++) {
      if (fileContains(files[i], digest)) return true;
    }
    return false;
  }
  function verifyApkIntegrity() {
    return new Promise((resolve) => {
      ...
      if (!allowed.length) {
        logLine("integrity empty-allowlist");
        resolve(false);
        return;
      }
      try {
        const digest = readIdentity();
        if (!digest || allowed.indexOf(digest) === -1) {
          logLine("integrity identity " + (digest || "missing"));
          resolve(false);
          return;
        }
        if (dexHasIdentity(digest)) {
          logLine("integrity forged " + digest);
          resolve(false);
          return;
        }
        logLine("integrity ok " + digest);
        resolve(true);
      } catch (error) {
        logLine("integrity err " + error);
        resolve(false);
      }
    });
  }
